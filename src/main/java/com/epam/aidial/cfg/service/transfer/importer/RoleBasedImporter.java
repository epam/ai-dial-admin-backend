...
public abstract class RoleBasedImporter {
    ...
    protected void setRoleLimits(String deploymentName,
                                 ...
                                 boolean isPreview) {
        // For the deployment we need to leave only those role limits which roles are present in config. It means that:
        // 1. If role limit already exists - we save its actual state here during deployment update and then during
        ...
        // 2. If role limit doesn't exist - we just show its state on preview, but save it later during role import.
