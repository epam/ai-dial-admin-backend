...
public class InfluxEngine implements Engine {

    private final InfluxDatasetDeclaration declaration;
    ...
    private Table getTable(InfluxTableDeclaration tableDeclaration) {
    ...
    // TODO: implement
    //    public Schema getDynamicSchema(String tableName) {
    //        var source = getTableSource(tableName);
    //        var query = """
    ...
    //                )
    //                |> sort()
    //                """.formatted(source.getBucket(), source.getMeasurement());
    //        log.debug("Query for retrieving schema was generated. Table: {}. Query: {}", tableName, query);
    //
    //        var columnNames = retrieveStringValues(query).stream()
    //                .filter(columnName -> !MEASUREMENT_COLUMN.equals(columnName))
    //                .toList();
    //
    //        var fields = columnNames.stream()
    //                .map(columnName -> new Field(columnName, FieldType.STRING))
    //                .collect(Collectors.toCollection(ArrayList::new));
    //        fields.add(new Field(TIME_COLUMN, FieldType.DATETIME));
    ...
    //    @Override
    //    public FieldAvailability getFieldAvailability(String tableName, String fieldName) {
    //        var source = getTableSource(tableName);
    //        var query = """
    //                from(bucket: "%1$s")
    ...
    //                |> distinct(column: "%3$s")
    //                |> sort()
    //                """.formatted(source.getBucket(), source.getMeasurement(), fieldName);
    //        log.debug("Query for retrieving field availability was generated. Table: {}. Field: {}. Query: {}",
    //                tableName, fieldName, query);
    //
    //        var values = retrieveStringValues(query);
    //        return new FieldAvailability(fieldName, FieldType.STRING, values);
    //    }
