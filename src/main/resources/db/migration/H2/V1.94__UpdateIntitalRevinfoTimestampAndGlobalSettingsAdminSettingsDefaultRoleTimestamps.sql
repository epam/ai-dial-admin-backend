...
-- but it must not used CURRENT_TIMESTAMP, since it differs from timestamp when initial revision was actually created.

-- Since created_at_ms of 'default' role was not updated in In V1.81__UpdateIntitalRevinfoTimestampAndGlobalSettingsTimestamps.sql,
...
-- possibly shifted epoch millis
SELECT CAST(DATEDIFF('MILLISECOND', TIMESTAMP '1970-01-01 00:00:00', CURRENT_TIMESTAMP) AS BIGINT);

SET @epoch_timestamp = SELECT created_at_ms + @shift FROM role_entity WHERE name = 'default';
UPDATE revinfo SET timestamp = @epoch_timestamp WHERE id = 1;

-- Update created_at_ms for adapter entities which latest creation is at initial revision
...
-- Update created_at_ms for addon entities which latest creation is at initial revision
UPDATE addon_entity
SET created_at_ms = (SELECT timestamp FROM revinfo WHERE id = 1)
...
-- Update created_at_ms for admin settings entities which latest creation is at initial revision
...
-- Update created_at_ms for application entities which latest creation is at initial revision
...
-- Update created_at_ms for application type schemas entities which latest creation is at initial revision
...
-- Update created_at_ms for assistant entities which latest creation is at initial revision
...
-- Update created_at_ms for global settings entities which latest creation is at initial revision
...
-- Update created_at_ms for interceptor entities which latest creation is at initial revision
...
-- Update created_at_ms for interceptor runner entities which latest creation is at initial revision
...
-- Update created_at_ms for key entities which latest creation is at initial revision
UPDATE key_entity
SET created_at_ms = (SELECT timestamp FROM revinfo WHERE id = 1)
WHERE name IN (
    SELECT name
...
-- Update created_at_ms for model entities which latest creation is at initial revision
UPDATE model_entity
SET created_at_ms = (SELECT timestamp FROM revinfo WHERE id = 1)
WHERE deployment_name IN (
    SELECT deployment_name
...
-- Update created_at_ms for role entities which latest creation is at initial revision
UPDATE role_entity
SET created_at_ms = (SELECT timestamp FROM revinfo WHERE id = 1)
WHERE name IN (
    SELECT name
...
-- Update created_at_ms for route entities which latest creation is at initial revision
UPDATE route_entity
SET created_at_ms = (SELECT timestamp FROM revinfo WHERE id = 1)
WHERE deployment_name IN (
    SELECT deployment_name
...
-- Update created_at_ms for toolset entities which latest creation is at initial revision
...
-- Update epoch_timestamp_ms for audit activity entities which revision = 1
