...
UPDATE adapter_entity e SET created_at_ms = (
    SELECT r.timestamp FROM adapter_entity_aud ea
...
UPDATE adapter_entity e SET updated_at_ms = (
    SELECT r.timestamp FROM adapter_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
...
UPDATE route_entity e SET created_at_ms = (
    SELECT r.timestamp FROM route_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
...
UPDATE route_entity e SET updated_at_ms = (
    SELECT r.timestamp FROM route_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
...
UPDATE interceptor_runner_entity e SET created_at_ms = (
    SELECT r.timestamp FROM interceptor_runner_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
    WHERE ea.name = e.name AND ea.revtype = 0
    ORDER BY r.id DESC
    LIMIT 1
...
UPDATE interceptor_runner_entity e SET updated_at_ms = (
    SELECT r.timestamp FROM interceptor_runner_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
    WHERE ea.name = e.name AND ea.revtype != 2
    ORDER BY r.id DESC
...
UPDATE application_type_schema_entity e SET created_at_ms = (
    SELECT r.timestamp FROM application_type_schema_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
...
UPDATE application_type_schema_entity e SET updated_at_ms = (
    SELECT r.timestamp FROM application_type_schema_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
    WHERE ea.schema_id = e.schema_id AND ea.revtype != 2
    ORDER BY r.id DESC
    LIMIT 1
) WHERE e.updated_at_ms IS NULL;
...
UPDATE key_entity e SET updated_at_ms = (
    SELECT r.timestamp FROM key_entity_aud ea
    JOIN revinfo r ON ea.rev = r.id
    WHERE ea.name = e.name AND ea.revtype != 2
    ORDER BY r.id DESC
    LIMIT 1
