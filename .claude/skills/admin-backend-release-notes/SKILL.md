...
| A `## CI` section emitted by the CI (e.g. a `ci(deps):` PR title)                                                                                                                                     | `Other`                                                                                                                                                                                                                                                                                             | The CI grew a `CI` heading the polished releases do not have; fold these in with the other maintenance items.                  |
...
5. **Describe behavior, not internals.** Name what a consumer touches: config fields, env vars, endpoint paths, enum values, schema types. Do **not** name internal implementation types (mappers, validators, services, DTOs) in a bullet — the classes that carry the change belong in the editorial-notes file, not the release notes.
6. **Backticks for code identifiers**: env vars, file paths, config field names, class names consumers reference, endpoint paths, schema keys.
7. **Issue ref at the start, PR ref(s) at the end.** See format above. Extract the issue number from the PR body (see Step 2, point 3).
...
# Behavior over internals — implementation classes stay out of the bullet:
- * fix: preserve upstream id on core config import (#1144)
+ *   [#1143](url) Preserved the upstream `id` on core-format config import ([#1144](url))
#   (not "... config import: `ModelCoreMapper` and `RouteCoreMapper` no longer drop it")
