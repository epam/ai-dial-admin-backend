...
| core.auth.token.provider.useCache                       | CORE_AUTH_TOKEN_PROVIDER_USE_CACHE                         | true    | No       | config.autoReload.enabled=true         | Enables or disables caching of auth tokens. If set to `true`, the token will be cached and reused until it is close to expiration. If set to `false`, a new token will be requested for each authentication attempt.                                          |
| core.auth.token.provider.refreshBeforeExpirationSeconds | CORE_AUTH_TOKEN_PROVIDER_REFRESH_BEFORE_EXPIRATION_SECONDS | 600     | No       | core.auth.token.provider.useCache=true | Specifies the number of seconds before a token’s expiration when the cached token should be refreshed. If the remaining lifetime of the cached token is less than this value, the token provider will request a new token instead of using the cached one.    |
