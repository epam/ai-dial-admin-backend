...
| Setting                                                 | Environment Variable                                       | Default | Required | Applied when                           | Description                                                                                                                                                                                                                                                   |
|---------------------------------------------------------|------------------------------------------------------------|---------|----------|----------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| core.auth.token.provider.url                            | CORE_AUTH_TOKEN_PROVIDER_URL                               | -       | Yes      | -                                      | Specifies the URL of the authentication token provider (e.g., Azure, Keycloak). This is the endpoint where the admin backend will request authentication tokens to interact with the DIAL Core. Important: Admin access must be configured for this provider. |
| core.auth.token.provider.clientId                       | CORE_AUTH_TOKEN_PROVIDER_CLIENT_ID                         | -       | Yes      | -                                      | Represents the client ID used to authenticate with the authentication token provider. This ID is assigned to the admin backend by the auth token provider.                                                                                                    |
| core.auth.token.provider.clientSecret                   | CORE_AUTH_TOKEN_PROVIDER_CLIENT_SECRET                     | -       | Yes      | -                                      | The client secret associated with the client ID, used to authenticate and authorize the admin backend with the authentication token provider. This secret should be kept secure.                                                                              |
| core.auth.token.provider.scope                          | CORE_AUTH_TOKEN_PROVIDER_SCOPE                             | -       | No       | -                                      | Specifies the scope of access requested by the admin backend. This can define the level of access or specific permissions the token should grant. If not specified, it defaults to an empty value.                                                            |
| core.auth.token.provider.useCache                       | CORE_AUTH_TOKEN_PROVIDER_USE_CACHE                         | true    | No       | -                                      | Enables or disables caching of auth tokens. If set to `true`, the token will be cached and reused until it is close to expiration. If set to `false`, a new token will be requested for each authentication attempt.                                          |
