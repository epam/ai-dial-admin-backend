...
| core.auth.token.provider.contentType  | CORE_AUTH_TOKEN_PROVIDER_CONTENT_TYPE   | application/x-www-form-urlencoded | No       | -            | Defines the content type for the token request. By default, it is set to `application/x-www-form-urlencoded`, which is commonly used by most providers.                                                                                                       |
| core.auth.token.provider.grantType    | CORE_AUTH_TOKEN_PROVIDER_GRANT_TYPE     | client_credentials                | No       | -            | Indicates the grant type used for obtaining the authentication token. The default value is `client_credentials`, which is used for server-to-server authentication.                                                                                           |
