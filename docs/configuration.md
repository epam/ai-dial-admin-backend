...
| config.autoReload.enabled                    | ENABLE_CONFIG_AUTO_RELOAD                      | false                | No       | -                                                | Enable the automatic scheduling of calls to the /reload_config endpoint on core. `core.auth.token.provider.*` must be configured in this way. |
